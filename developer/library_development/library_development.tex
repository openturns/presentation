...
\begin{frame}
  \frametitle{The CMake compilation infrastructure}
  \begin{block}{CMake usage}
    \begin{tabular}{l}
\ttfamily mkdir build \&\& cd build\\
\ttfamily cmake -DCMAKE\_BUILD\_TYPE=RelWithDebInfo\\
\ttfamily ~~~~~ -DCMAKE\_INSTALL\_PREFIX=\$PWD/install\\
\ttfamily ~~~~~ -DCMAKE\_CXX\_FLAGS="-Wall -Wextra -D\_GLIBCXX\_ASSERTIONS"\\
\ttfamily ~~~~~ -DCMAKE\_UNITY\_BUILD=ON -DCMAKE\_UNITY\_BUILD\_BATCH\_SIZE=16\\
\ttfamily ~~~~~ -DPYTHON_EXECUTABLE=/usr/bin/python3\\
\ttfamily ~~~~~ -DSWIG\_COMPILE\_FLAGS="-O1"\\
\ttfamily ~~~~~ -DUSE_SPHINX=ON -DSPHINX\_FLAGS="-W -T -j4"\\
\ttfamily ~~~~~ ..
\ttfamily \\
\ttfamily make install -j4\\
\ttfamily ctest -R pyinstall -j4\\
\ttfamily \\
\ttfamily make tests -j4\\
\ttfamily ctest -R cppcheck -j4\\
\end{tabular}
\end{block}
\end{frame}
