...
\begin{figure}
\centering
\includegraphics{../images/MethodologieIncertitude-EN_arial.png}
...
\end{itemize}
    ...
    \begin{figure}
\centering
\includegraphics{../images/wrapper-python-benchmark.png}
...
can make the evaluation faster. - \texttt{func\_sample}: evaluate the
function on a \texttt{Sample} instead of a \texttt{Point} to vectorize
the evaluations ; - \texttt{n\_cpus}: uses \texttt{multiprocessing} to
make the evaluations parallel.

    Here are different cases, depending the options specified by the user: -
If \texttt{func\_sample} is undefined and \texttt{n\_cpus} is undefined
...
- If \texttt{func\_sample} is undefined and \texttt{n\_cpus} is defined,
then the implementation uses \texttt{multiprocessing}'s \texttt{Pool} to
make the evaluation parallel (see
...
- If \texttt{func\_sample} and \texttt{n\_cpus} are both defined, then
the implementation uses divides the \texttt{Sample} into sub-samples
which are evaluated in parallel (see
...
parameter (whatever its type) to the evaluation. - The constructor of
the object can have any number or type of input arguments, as any
\texttt{Class} object in Python. - The calculations which are done in
...
\end{Shaded}
    ...
    \begin{figure}
\centering
\includegraphics{../images/benchmark_Morris_otwrapy.png}
\caption{Python benchmark}
\end{figure}
...
The two mandatory methods are: - \texttt{getRange()}; -
\texttt{computePDF()}.
