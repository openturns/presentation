...
plt.plot(np.zeros(y.getSize()),y,'*', ms = 14)
...
algo.setThreshold(1e-4)  # we expect to explain 96% of variance
algo.setRecompress(
    False
)  # whether to re-truncate modes according to a global eigen value threshold across inputs (default)
algo.setNbModes(40)  # max KL modes (default=unlimited)
# 2. chaos parameters:
...
for i in range(4):
    grph_ = kl_results[i].getModesAsProcessSample().draw()
    grph_ = grph_.getGraphCollection()[0]
    grph_.setTitle('Modes')
    _ = View(grph_)
